...
const CHANNELS = [
  { icon: "fa-comments", title: "Chat with Support", note: "Live, 8 AM – 8 PM (Mon–Sat)", cta: "Open chat", to: "/messages" },
  { icon: "fa-phone-volume", title: "Helpline", note: "+977 1-4XXXXXX · toll-free 1660-X", cta: "Call now", to: "/help" },
  { icon: "fa-envelope", title: "Email & Ticket Desk", note: "support@gulmelifancystore.np · replies < 24h", cta: "Send message", to: "/help" },
  { icon: "fa-store", title: "Experience Center", note: "Gulmeli bazaar counter, walk-ins welcome", cta: "Visit us", to: "/" },
];
    ...
    <div className="space-y-8 pb-10">
      <header className="rounded-[2px] bg-gradient-to-r from-[#d82a0b] via-[#f85606] to-[#fc8621] p-8 text-white shadow-sm">
        ...
        <p className="mt-2 max-w-xl text-sm text-white/85">
          Search the FAQs, chat with our team, or drop by the experience
          center. Merchant selling on {">"} the store?{" "}
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {CHANNELS.map((c) => (
          <div
            key={c.title}
            className="daraz-card flex flex-col justify-between rounded-[2px] border border-gray-200 bg-white p-4 shadow-sm"
          >
            <div>
              <span className="grid h-10 w-10 place-items-center rounded-full bg-orange-50 text-[#f85606]">
                <i className={`fa-solid ${c.icon}`} />
              </span>
              <h3 className="mt-3 text-sm font-black text-gray-800">{c.title}</h3>
              <p className="mt-1 text-[11px] leading-snug text-gray-500">{c.note}</p>
            </div>
            <Link
              to={c.to}
              className="mt-3 inline-block rounded-[2px] border border-[#f85606] px-3 py-1.5 text-center text-[11px] font-bold uppercase text-[#f85606] hover:bg-orange-50"
            >
              {c.cta}
            </Link>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <section className="lg:col-span-2">
          <h2 className="mb-3 text-lg font-bold text-gray-800">Top FAQs &amp; Solutions</h2>
          ...
          <div className="overflow-x-auto rounded-[2px] border border-gray-200 bg-white shadow-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {dexHubs.map((h) => (
                  <tr key={h.code} className="hover:bg-orange-50/40">
                    <td className="px-4 py-2.5 font-semibold text-gray-800">{h.hub}</td>
                    <td className="px-4 py-2.5 text-gray-500">{h.province}</td>
                    <td className="px-4 py-2.5 font-mono text-gray-500">{h.code}</td>
                    <td className="px-4 py-2.5 text-gray-500">8:00 – 18:00, Sun–Fri</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Contact form */}
        <section>
          <h2 className="mb-3 text-lg font-bold text-gray-800">Send Us a Direct Message</h2>
          <form
            className="space-y-3 rounded-[2px] border border-gray-200 bg-white p-4 shadow-sm"
            onSubmit={(e) => {
              e.preventDefault();
              setSent(true);
            }}
          >
            {sent ? (
              <div className="flex flex-col items-center gap-2 py-8 text-center">
                ...
                <p className="text-sm font-black text-gray-800">Message dispatched!</p>
              ...
              <>
                ...
                <label className="block text-xs font-semibold text-gray-500">
                  ...
                  <select
                    ...
                    className="mt-1 w-full rounded border border-gray-200 px-3 py-2 text-sm font-normal outline-none focus:border-[#f85606]"
                  >
                    {["Orders & delivery", "Returns & refunds", "Payments", "Selling on the store", "Other"].map((t) => (
                ...
                <button
                  type="submit"
                  className="w-full rounded-[2px] bg-[#f85606] py-2.5 text-sm font-black uppercase text-white hover:bg-[#d04402]"
                >
                  Send message
                </button>
              </>
          ...
          <div className="mt-4 rounded-[2px] border border-gray-200 bg-white p-4 text-xs text-gray-500 shadow-sm">
